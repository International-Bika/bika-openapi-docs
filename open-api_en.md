...
BIKA provide powerful and robust APIs to help user write program to automatic the tradings. With this guide you can learn how to use APIs.
...
* The base endpoint is [https://openapi.bika.live /api/v2](https://openapi.bika.live /api/v2)
...
Before get start, you need get the api key and secret first. The 'api key' is a unique string BIKA allocated for you (you can think it as the speicial username used in api) and the 'secret' is the private key used to sign your request data(and you can think it as the password for api).

You can get the apiKey and secret key from BIKA website : https://www.bika.pro.
...
BIKA provides both REST API and Websocket API :
...
```bash
curl https://openapi.bika.live /api/v2/time
```
...
| pair | string | true | BIKA supports trade pairs|
...
```bash
curl https://openapi.bika.live /api/v2/trades?pair=ETH_BTC&total=3
```
...
|------|------|----------|-------------|
| key | string | true | apiKey of the user |
| pair | string | false | BIKA supports trade pairs |
...
```bash
curl -H "Content-Type: application/json" -x POST https://openapi.bika.live /api/v2/userinfo --data '{"key":"VmhrcrQEF3ATxV2JtVMEH4dFpGEmYzixOL4VrvAeR2COXtc9pzXbvFV1jLbFXEQf","timestamp":1536323160000,"sign":"c69d8ec9e274dd20126972b2dfaedc8c74cf06fbb19f968eedcc0a300a95b9f6"}'
```
...
```bash
curl https://openapi.bika.live /api/v2/ticker?pair=ETH_BTC
```
...
| name | type | required | description |
|------|------|----------|-------------|
| pair | string | true  | BIKA supports trade pairs. Returns a specific trade against the market when specifying a pair,Returns all trading to market prices without specifying pair;|
...
```bash
curl https://openapi.bika.live /api/v2/depth?pair=ETH_BTC&size=5&merge=8
```
...
```bash
curl https://openapi.bika.live /api/v2/kline?pair=ETH_BTC&period=1min
```
...
```bash
curl https://openapi.bika.live /api/v2/pairs
```
...
```bash
curl https://openapi.bika.live /api/v2/pairLimits?pair=ETH_BTC
```
...
```bash
curl https://openapi.bika.live /api/v2/pairRate
```
...
The request data format is Json type, such as {"key":"123456789","exchange","BIKA","timestamp": 1545347383600}.
...
```bash
POST https://openapi.bika.live /api/v2/getSign
```
...
```bash
curl https://openapi.bika.live /api/v2/pairList?pair=ETH_BTC
```
...
```bash
curl https://openapi.bika.live /api/v2/coinList
```
