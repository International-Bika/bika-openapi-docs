...
BIKA为用户提供一整套简单而又强大的开发工具，旨在帮助用户快速、高效地将BIKA行情和交易功能整合到自己的应用当中。
BIKA接口是提供服务的基础，开发者在BIKA网站创建账号后，可以根据自身需求建立不同权限的API，并利用API进行自动交易。
通过API可以快速实现以下功能：
...
开启API权限
用户的API权限在BIKA平台的[个人中心]->[我的API]获取。点击申请API即可获得，其中key是BIKA提供给API用户的访问密钥，secretKey用于对请求参数签名的私钥。
注意： 请勿向任何人泄露这两个参数，这两个参数关乎您账号的安全。

**三、调用模式**

BIKA为用户提供了两种调用接口的方式，开发者可根据自己的使用场景和偏好，选择适合自己的方式来查询行情、进行交易。
...
REST访问的根URL：https://openapi.bika.live /api/v2 
...
> Request
```java
  GET https://openapi.bika.live /api/v2/time
```
...
```java
POST https://openapi.bika.live /api/v2/placeOrder
```
...
orderProperty	Integer		否			订单属性:(普通:0; FAK:1; FOK:2; PostOnly:3)
...
```java
POST https://openapi.bika.live /api/v2/cancelOrder
```
...
```java
POST https://openapi.bika.live /api/v2/orderInfo
```
...
```java
POST https://openapi.bika.live /api/v2/orderList
```
> Response
...
```java
POST https://openapi.bika.live /api/v2/orderHistory
```
...
```java
POST https://openapi.bika.live /api/v2/beforeOrderHistory
```
...
```java
GET https://openapi.bika.live /api/v2/trades?pair=ETH_BTC
```
...
```java
POST https://openapi.bika.live /api/v2/tradesHistory
```
...
```java
POST https://openapi.bika.live /api/v2/myTrades
```
...
```java
POST https://openapi.bika.live /api/v2/userinfo
```
...
```java
GET https://openapi.bika.live /api/v2/ticker?pair=ETH_BTC
```
...
```
参数名		参数类型	必填							 描述
pair		String	否    BIKA支持的交易对。指定pair时返回特定交易对市场行情；未指定pair时返回全部交易对市场行情；
```
### 13．orderbook
> 说明
```
...
```java
GET https://openapi.bika.live /api/v2/depth?pair=ETH_BTC
```
...
```java
GET https://openapi.bika.live /api/v2/kline?pair=ETH_BTC&period=1min
```
...
```java
  GET https://openapi.bika.live /api/v2/pairs
```
...
```java
GET https://openapi.bika.live /api/v2/pairLimits?pair=ETH_BTC
```
...
```java
GET https://openapi.bika.live /api/v2/pairRate
```
...
```
  ...
  请求数据格式为Json类型，譬如：{"key":"123456789","exchange":"BIKA","timestamp":1545347383600}
